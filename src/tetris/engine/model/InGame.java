...
import tetris.engine.observer.ScoreCounter;
import tetris.engine.observer.ScoreObserver;
import tetris.engine.view.screen.GameScr;
...
public class InGame {
	...
	private int score;
	private ScoreObserver scoreObserver = new ScoreCounter();
	public InGame(GameScr gameScr, int level) {
		...
		score = 0;
		board = new Board();
		board.addScoreObserver(scoreObserver);
		setNextShape();
		setCurrentShape();
	}
	...
	public void update() {
		...
		notifyScoreChanged(score);
		  System.out.println("Score: " + score); // check 
	}
	...
	private void setNextShape() {
		...
		for (Block block : nextShape.getBlocks()) {
			nextShapeBlocks.add(new Block(block.getColor(), block.getX() - nextShape.getX() + 11,
					block.getY() - nextShape.getY() + 2));
		}
	}
